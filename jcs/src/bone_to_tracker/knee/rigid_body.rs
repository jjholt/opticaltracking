...
impl<const ID: usize> RigidBody<ID> {
    pub fn new(side: Side, medial: ProbeData, lateral: ProbeData, far_landmark: ProbeData, tracker: ProbeData) -> RigidBody<ID> {
...
mod test {
    ...
    fn tracker_transform() {
        ...
        let side = Side::Right;
        let fm = ProbeRawData::new(0.8228, 0.1357, 0.4408, -0.3318, 15.3196, -54.9971, -2097.6023).into();
        let fl = ProbeRawData::new(0.4031, 0.4746, 0.4195, -0.6603, 16.9156, 16.2064, -2059.3142).into();
        let fp = ProbeRawData::new(0.4280, 0.4662, 0.4471, -0.6319, -8.5689, 15.8874, -2131.4353).into();
